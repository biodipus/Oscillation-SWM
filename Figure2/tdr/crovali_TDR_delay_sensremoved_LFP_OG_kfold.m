...
%     clearvars -except alldays dayid exp_ratio enc_weight_all monkey
    ...
    if strcmp(monkey, 'ocean')
        ...
        if sensdim == 5
            savefile = ['C:\Wen\Project\Osc_control\Data\Result_ocean\LFP\tdr_frontal_L2_croval\tdr_mem_delay1_5dim\',...
                days(1:end),'_weight_mem_delay_', osc_band{bandid}, '_CV.mat'];
        else
            savefile = ['C:\Wen\Project\Osc_control\Data\Result_ocean\LFP\tdr_frontal_L2_croval\tdr_mem_delay1\',...
                days(1:end),'_weight_mem_delay_', osc_band{bandid}, '_CV.mat'];
        end
    end
    ...
    if strcmp(monkey, 'grootsw')
        ...
        if sensdim == 5
            savefile = ['C:\Wen\Project\Osc_control\Data\Result_grootsw\LFP_sw\tdr_frontal_L2_croval\tdr_mem_delay1_5dim\', days(1:end),'_weight_mem_delay_', osc_band{bandid}, '_CV.mat'];
        else
            savefile = ['C:\Wen\Project\Osc_control\Data\Result_grootsw\LFP_sw\tdr_frontal_L2_croval\tdr_mem_delay1\', days(1:end),'_weight_mem_delay_', osc_band{bandid}, '_CV.mat'];
        end
        if strcmp(ruletype,'repeat') || strcmp(ruletype,'mirror')
            time = [70:75];
            savefile = ['C:\Wen\Project\Osc_control\Data\Result_grootsw\LFP_sw\tdr_frontal_L2_croval\tdr_mem_delay2_5dim\',ruletype, days(1:end),'_',osc_band{bandid},...
    ...
    %%%% plot acc
    validationAccuracy = mean(validationAccuracy,4);
    validationAccuracy = mean(validationAccuracy,3);
    validationAccuracy_all(:,:,dayid) = validationAccuracy;
    plot(validationAccuracy(:,1),'Color','b');
    hold on
    plot(validationAccuracy(:,2),'Color','r');
