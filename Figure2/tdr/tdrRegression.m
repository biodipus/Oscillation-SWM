...
%     nrg = 10;
    ...
    % Initialize
%     bb = zeros(nun,npt,nrg); % regression coefficients
    bb = zeros(nun,npt,10); % regression coefficients
    ...
    % 回归自变量中心化
%     regmod(:,[2,3]) = regmod(:,[2,3])-mean(regmod(:,[2,3]),1);
%     regmod(:,4) = regmod(:,2).*regmod(:,3);
    % Normalize the regressors
    switch regpars.regressor_normalization
        case 'none'
            regnrm = regmod;
