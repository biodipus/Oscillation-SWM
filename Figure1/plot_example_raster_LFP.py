...
# file = '0710_WB57_TH_HG_mats'# gamma memory s2
...
with plt.style.context(style_path):
    ...
    vmin, vmax = 0.022, 0.023
    # vmin, vmax = 0.009, 0.010
    # vmin, vmax = 0.01, 0.015
    # cmap = plt.cm.gray
    cmap = plt.cm.inferno
...
file_name = r'C:\Wen\Project\Osc_control\Data\Data_ocean\ocean_pos_labels_0915_2021.mat'
location = sio.loadmat(file_name)['RAS_sorted']
ch_id = sio.loadmat(file_name)['chan_idx_sorted']
location = np.concatenate((location, ch_id), axis=1)

ch_info_path = r'C:\Wen\Project\Osc_control\Data\Data_ocean'
ele_sit = pd.read_excel(ch_info_path + '\\Ocean电极位置_adjusted.xlsx','all')
ele_sit = ele_sit.drop(columns = 'bd area')
ele_sit = ele_sit.loc[ele_sit['area'].isin(['PFC','PM'])].reset_index()

idx = np.isin(location[:,3], ele_sit['ch'])
location = location[idx,:]

with plt.style.context(style_path):
    colors = plt.rcParams["axes.prop_cycle"].by_key()["color"][0:]
    fig, axs = plt.subplots(1, 1, figsize=(1.2, 0.8), dpi=300) # , constrained_layout=True
    axs.scatter(location[:,0], location[:,1], c='k', s=10)
    ...
    figpath = r'C:\Wen\OneDrive\Project\Osc_control\Manuscript\Figures\Figure1'
    fig.savefig(figpath + '\\channle_loc.pdf', dpi='figure')
